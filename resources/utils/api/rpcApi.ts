...
const invokeFunc = (contractHash: string, method: string, params: any[], account?: string) => {
    const paramsInfo = account ?
        [contractHash, method, params, [{ account: account, scopes: 'CalledByEntry', allowedcontracts: [], allowedgroups: [] }]]
        :
    ...
    return pRetry(retryFetch, { retries: 2 }).then((res) => res.result).catch((err) => {
        console.log(err);
        return fetch(getAlternateNetwork(), fetchInitData).then((res) => res.json()).then((res) => res.result).catch((err) => console.log(err));
    });
}

const invokeScript = (script: string, account?: string) => {
    const paramsInfo = account ?
        [script, [{ account: account, scopes: 'CalledByEntry', allowedcontracts: [], allowedgroups: [] }]]
        :
        [script];
    const fetchInitData = {
        method: 'POST',
...
const getUnclaimedGAS = (address: string) => {
    ...
    return invokeFunc(constants.BNEO, 'reward', [{ type: "Hash160", value: scriptHash }]);
}
...
const getBalance = (asset: string, scripthash: string) => {
    return invokeFunc(asset, 'balanceOf', [{ type: 'Hash160', value: scripthash }]);
}
...
const getQuote = () => {
    ...
    }).then((res) => res.json()).then((res) => res).catch(() => {
...
async function burgerGasPerNeoPerSecond() {
    ...
    const start = `https://neoburger.blob.core.windows.net/data/${block - 8192}.json`;
    const stop = `https://neoburger.blob.core.windows.net/data/${block - 8192 - 131072}.json`;
    const retryFetch = async () => {
        const response = await Promise.all([fetch(start, { method: 'GET' }), fetch(stop, { method: 'GET' })])
        if (!response[0].ok) {
    ...
    const resp = await pRetry(retryFetch, { retries: 2 }).catch(async (err) => {
        console.log(err);
        const start2 = `https://raw.githubusercontent.com/neoburger/statistics/data/data/${block - 8192}.json`;
        const stop2 = `https://raw.githubusercontent.com/neoburger/statistics/data/data/${block - 8192 - 131072}.json`;
        return await Promise.all([fetch(start2, { method: 'GET' }), fetch(stop2, { method: 'GET' })]).catch((err) => {
            console.log(err);
            return null;
        });
    });
    ...
    const [jx, jy] = await Promise.all(resp.map(v => {
        return v.json();
    })).catch((err) => {
        console.log(err);
        return [null, null];
...
const getCurrentBlockHeight = () => {
    ...
    return pRetry(retryFetch, { retries: 2 }).then((res) => res.result).catch((err) => {
        console.log(err);
        return fetch(getAlternateNetwork(), fetchInitData).then((res) => res.json()).then((res) => res.result).catch((err) => {
            console.log(err);
            return null;
        });
    });
...
const getBlockBurgerInfo = (blockIndex: string) => {
    const retryFetch = async () => {
        const response = await fetch(`https://neoburger.blob.core.windows.net/data/${blockIndex}.json`, { method: 'GET' });
        if (!response.ok) {
            throw new Error(response.statusText);
        }
        return response.json();
    ...
    return pRetry(retryFetch, { retries: 2 }).catch((err) => {
        console.log(err);
        return fetch(`https://raw.githubusercontent.com/neoburger/statistics/data/data/${blockIndex}.json`, { method: 'GET' }).then((res) => res.json()).catch((err) => {
            console.log(err);
            return null;
        });
    });
...
const getAccountState = (asset: string, agenthash: string) => {
    return invokeFunc(asset, 'getAccountState', [{ type: 'Hash160', value: agenthash }]);
}
...
const getNoBugInfo = () => {
    const fetchInitData = {
        method: 'GET',
        headers: {
            Network: window.location.search.indexOf('dev') >= 0 ? 'testnet' : 'mainnet'
        }
    };
    return fetch(`https://api.neotube.io/v1/asset/${constants.NOBUG}`, fetchInitData).then((res) => res.json()).catch((err) => {
        console.log(err);
        return null;
    });
}
...
// nobug是否已经领取
const nobugIsClaimed = (scripthash: string, amount: string, nonce: string, proof: string[]) => {
    return invokeFunc(
        constants.NOBUG,
        'claim',
        [
            ...
            {
                type: "Array",
                value: proof.map((v) => { return { type: 'Hash256', value: v } })
            }
        ]
        );
}
...
// 获取投票状态
const votee = (address: string, id: string, forOrAgainst: string, unvote: string) => {
    const addressHash = `0x${wallet.getScriptHashFromAddress(address)}`;
    return invokeFunc(constants.BURGERGOV, 'vote', [
        {
            type: 'Hash160',
            value: addressHash,
        },
